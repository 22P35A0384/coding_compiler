...
// Function to check if the code requires input
const requiresInput = (code) => {
    const inputPatterns = /scanf|gets|fgets|read/;
    return inputPatterns.test(code);
};

// Function to execute C code test case
...
const executeTestCase1 = (code, input, expectedOutput, executablePath, isHidden = false) => {
    return new Promise((resolve) => {
        // Write the C code to a temporary file
        const filePath = path.join(__dirname, 'temp.c');
        fs.writeFileSync(filePath, code);
        ...
        let output = '';
        let responded = false;
        const timeout = 60000; // 60-second timeout for each test case
    ...
    try {
        ...
        if(requiresInput(code)) {
            ...
            for (let i = 0; i < hiddenInputOutputList.length; i++) {
                ...
                const result = await executeTestCase1(code, input, expectedOutput, executablePath, true);
                results.push({
                    result: result.result,
                    error: result.error || null,
        ...
        if (allPassed) {
            ...
            if (userData) {
                ...
                await Problem.findByIdAndUpdate(id, {
                    $addToSet: { solved_users: { user_id: userId, code, language: "C" } }, // Ensure no duplicate user entries
                });
            }
        }
